...
set terminal pngcairo size 400,200 font "Sans,12" transparent
...
plot "<nl 2021-07-692039-scripts.txt | grep single-sig" u (int($1) % mywidth):(floor($1 / mywidth)) title "Single-sig"\
   , "<nl 2021-07-692039-scripts.txt | grep 1-of-1" u (int($1) % mywidth):(floor($1 / mywidth)) title "1-of-1"\
   , "<nl 2021-07-692039-scripts.txt | grep 2-of-2" u (int($1) % mywidth):(floor($1 / mywidth)) title "2-of-2"\
   , "<nl 2021-07-692039-scripts.txt | grep 2-of-3" u (int($1) % mywidth):(floor($1 / mywidth)) title "2-of-3"\
   , "<nl 2021-07-692039-scripts.txt | grep 3-of-4" u (int($1) % mywidth):(floor($1 / mywidth)) title "3-of-4" \
...
set terminal pngcairo size 400,160 font "Sans,12" transparent

set output './2021-07-multisignature-fungible.png'
plot "<nl 2021-07-692039-scripts.txt | egrep '(single-sig|-of-)'" u (int($1) % mywidth):(floor($1 / mywidth)) title "Keypath"\
