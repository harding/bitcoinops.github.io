...
distinctively recorded to the block chain where surveillance companies
...
- **MuSig2**, also simple to implement and which eliminates one round of
  communication and allows another round to be combined with key
  exchange so that it's possible to use a somewhat similar signing
...
- **MuSig-DN** (Deterministic Nonce), significantly more complex to
  implement and its communication between participants can't be combined
  with key exchange.  However, it's not vulnerable to the repeated
  session attack.

All signers have to agree on the protocol to use, so there will likely
be a network effect where almost all implementations use the same
...
project to add MuSig2 support.  We expect the basic workflow will look
something like:

1. The wallet for each participant generates a [BIP32][] xpub that is shared
   with all the other participants (the same as is commonly done now for
   multisigs)
...
primary keys can use a multisignature for the taproot key-path spend (e.g.
...
separate branches of a [tapscript][topic tapscript].
...
spending still works as expected and remains relatively efficient.

Although users wanting minimal fees and maximal privacy may eventually
switch to pure threshold signature schemes, the above scheme may also
